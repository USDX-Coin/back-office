...
// ─────────────────────────────────────────────────────────────────────────────
// Reporting aggregators (sot/api/reporting.yaml)
//
// Backend aggregates on-the-fly from `mint_requests` / `burn_requests` — the
// MSW mock mirrors that by walking the same in-memory request list. Timezone
// for "per day" buckets is Asia/Jakarta per SoT.
// ─────────────────────────────────────────────────────────────────────────────
...
function jakartaDate(iso: string): string {
  return JAKARTA_DATE_FMT.format(new Date(iso))
}

// Decide whether a request row matches the common filter set (date range +
// chain + status + optional userId). Status filter is permissive — empty
// or undefined = no status filter (matches all).
function matchesReportFilter(
  ...
  const d = jakartaDate(r.createdAt)
  if (d < filter.startDate || d > filter.endDate) return false
...
export function aggregateDailyMint(
  requests: RequestListItem[],
  filter: ReportFilter
): DailyMintRow[] {
  ...
  for (const r of requests) {
    if (!matchesReportFilter(r, 'mint', filter)) continue
    const date = jakartaDate(r.createdAt)
    ...
    bucket.totalCount += 1
    bucket.sumUsdx += Number(r.amount)
    bucket.sumIdr += Number(r.amountIdr)
    if (r.status === 'PENDING_APPROVAL') bucket.countPendingApproval += 1
    else if (r.status === 'APPROVED') bucket.countApproved += 1
    else if (r.status === 'EXECUTED') bucket.countExecuted += 1
    else if (r.status === 'REJECTED') bucket.countRejected += 1
  }
  return [...byDate.entries()]
    .map(([date, b]) => ({
      date,
      totalCount: b.totalCount,
      totalAmountUsdx: b.sumUsdx.toFixed(6),
      totalAmountIdr: b.sumIdr.toFixed(2),
      countPendingApproval: b.countPendingApproval,
      countApproved: b.countApproved,
      countExecuted: b.countExecuted,
      countRejected: b.countRejected,
    }))
    .sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0))
}

export function aggregateDailyBurn(
  requests: RequestListItem[],
  filter: ReportFilter
): DailyBurnRow[] {
  const byDate = new Map<string, {
  ...
  for (const r of requests) {
    if (!matchesReportFilter(r, 'burn', filter)) continue
    const date = jakartaDate(r.createdAt)
    let bucket = byDate.get(date)
    if (!bucket) {
      bucket = {
    ...
    else if (r.status === 'IDR_TRANSFERRED') bucket.countIdrTransferred += 1
    else if (r.status === 'REJECTED') bucket.countRejected += 1
  }
  return [...byDate.entries()]
    .map(([date, b]) => ({
      ...
      countApproved: b.countApproved,
      countExecuted: b.countExecuted,
      countIdrTransferred: b.countIdrTransferred,
      countRejected: b.countRejected,
    }))
    .sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0))
}
...
export function aggregateByUser(
  ...
  for (const r of requests) {
    if (!matchesReportFilter(r, type, filter)) continue
    let bucket = byUser.get(r.userId)
    if (!bucket) {
      bucket = { userName: r.userName, totalCount: 0, sumUsdx: 0, sumIdr: 0 }
  ...
  return [...byUser.entries()]
    ...
    .sort((a, b) => Number(b.totalAmountUsdx) - Number(a.totalAmountUsdx))
}
