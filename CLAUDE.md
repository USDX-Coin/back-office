...
| `/kyb/:id` | — | — | All roles | KYB detail modal — entity block, UBO cards (`identityNumber` is PII → ADMIN only), five FIXED document slots **read-only** (the presign + attach endpoints exist since backend PR #275 but the upload UI is a follow-up, so an operator-entered record has every slot empty and approve can only answer `409 KYB_DOCUMENTS_INCOMPLETE` — no record reaches VERIFIED from this UI yet), approve / **reject with a MANDATORY reason of at least 10 characters** (`RejectKybDto @MinLength(10)` plus two DB CHECKs), enforced in the dialog, again in `useRejectKyb`, and again by the API — a dialog-only guard is bypassed by any other caller and the trail then carries a reasonless rejection |
